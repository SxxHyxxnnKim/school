...
<div class="container">
	...
	<div class="row pt-2">
		<h4>
			신청한 과목 개수: <span class="text-info"> ${countAll }</span> 신청학점:<span
				class="text-info"> ${totalCredit }</span>
		</h4>
	</div>
					...
					<c:if test="${rList ne null and not empty rList }">
						<c:forEach var="list" items="${rList }">
							<tr>
								...
								<td>${list.classnum }</td>
								<td>${list.credit }</td>
								<td><c:if test="${list.rstate eq 0 }">확인중</c:if> <c:if
										test="${list.rstate  eq 1 }">확정</c:if></td>
								<td><c:if
										test="${list.rstate eq 0 }">
